...
### 🚀 Démarrage Rapide (Script PowerShell)

**Méthode la plus simple - Script PowerShell unifié !**
...
- **1. Build RELEASE** - Compile en mode optimisé
- **2. Build DEBUG** - Compile avec logs de débogage
- **3. Run RELEASE** - Lance l'application en mode release
- **4. Run DEBUG** - Lance l'application en mode debug
- **5. Build et Run RELEASE** - Build puis lance
- **6. Build et Run DEBUG** - Recommandé pour le développement
- **7. Build GITHUB RELEASE** - Crée une release complète avec latest.json

Ou utiliser directement en ligne de commande :
```powershell
.\rustfocus.ps1 build       # Build release
.\rustfocus.ps1 debug       # Lance en mode debug
.\rustfocus.ps1 release     # Crée une GitHub release
```
...
```
src-tauri/target/release/organizer-2-0.exe
```

Note : Pour créer une release GitHub, utilisez plutôt `.\rustfocus.ps1 release` qui génère automatiquement les fichiers nécessaires.
...
# Gestionnaire de tâches > Détails > organizer-2-0.exe
